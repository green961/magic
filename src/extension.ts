...
import { Base, determineLang, lineEndings } from './utility/base'
...
export function activate(ctx: vscode.ExtensionContext) {
  ...
  vscode.commands.registerTextEditorCommand('wonderland.insertSemicolon', (editor, edit) => {
    ...
    function insertSemiAtEnd(lineIndex: number) {
      ...
      if (textTrimEnd) {
        ...
        if (textTrimEnd.lastIndexOf(' // ') !== -1) {
          return textTrimEnd.lastIndexOf(semi, textTrimEnd.lastIndexOf(' // '))
        } else if (textTrimEnd[pos] !== semi) {
  ...
  vscode.commands.registerTextEditorCommand('wonderland.PropertyInjection', async (editor) => {
    ...
    editor.edit((editBuilder) => {
      let praa = /\((.*)\)/.exec(text)[1]
      let param = `@Inject(${praa.split(':')[1].trim()})`

      const str = `\n` + [praa, param].reduceRight((s, e) => `${s}${' '.repeat(indentSize)}${e}\n`, '')

      editBuilder.insert(selection.active.with({ character: Infinity, line: lineNumber }), str)
    })
  })

  vscode.commands.registerTextEditorCommand('wonderland.apple', (editor) => {
    let relativePath = path.relative(vscode.workspace.rootPath, editor.document.uri.fsPath)

    // # python -m my_package.subpackage.module_c
    let str = `python -m ${relativePath.replace(/\.py$/, '').replaceAll('\\', '.')}`
    vscode.env.clipboard.writeText(str)
  })

  vscode.commands.registerTextEditorCommand('wonderland.ConvertKWArgsIntoDict', (editor, edit) => {
    ...
    edit.replace(
      ...
      `**{${selectionText
        .split(/\s*,\s*/)
        .map((prop) => {
          let keywordParam = prop.split('=')
          return `'${keywordParam[0]}': ${keywordParam[1]}`
        })
        .join(', ')}}`
    )
  })
  ...
  ctx.subscriptions.push(
    vscode.commands.registerTextEditorCommand('wonderland.Uncapitalize', async (editor, edit) => {
      ...
      // uploac(@UploadedFile() aa: Express.Multer.File) {} 子属性
      ...
      const lineIndex = selection.active.line
      const { text } = document.lineAt(lineIndex)
      const re = new RegExp(String.raw`(\w+)\s*:[a-zA-Z.\s]*${word}`)
      const result = re.exec(text)
      if (langObject.languageId === 'typescript' && result) {
  ...
  ctx.subscriptions.push(
    vscode.commands.registerTextEditorCommand('wonderland.ctrlPlusq', (editor, edit) => {
      ...
      if (langObject.languageId === 'csharp') {
        ...
        if (langObject.singCommRE.test(text)) {
          text = text.replace(langObject.singCommRE, '')
        }
  ...
  ctx.subscriptions.push(
    vscode.commands.registerTextEditorCommand('wonderland.ctrlPlusn', (editor, edit) => {
      ...
      let langObject: Base
      if (id === 'properties' && path.extname(editor.document.fileName) === '.conf')
        langObject = getInstance('conf')
      else langObject = getInstance(id)
