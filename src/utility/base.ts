...
export class Base {
  ...
  constructor(public singleLineComment: string, readonly languageId: Language) {
    ...
    this.singCommRE = new RegExp(String.raw`(?<=^\s*)(?:${this.singleLineComment}\s*)`)
  }
    ...
    if (selections.length === 2) {
      ...
      if (t0 || t1) {
        if (t0 && t1) {
          ...
          //   username: string
          //   content: string
          //   createdAt?: Date 选中 createdAt
          // }
          // export interface Messagc{
          //   id?:number
          //   username:string
          // 插入到此位置 插入到这个空行下面不适用
          // }

          // if (document.lineAt(emptyActive.line).isEmptyOrWhitespace) {
          //   const emptyAfterIndex = emptyActive.line + 1
          //   edit.insert(new vscode.Position(emptyAfterIndex, 0), value)

          //   const nonWhitespace = new vscode.Position(
          //     emptyAfterIndex,
          //     document.lineAt(emptyAfterIndex).firstNonWhitespaceCharacterIndex
          //   )
          //   editor.selection = new vscode.Selection(nonWhitespace, nonWhitespace)
          // }

          const emptyActive = isEmpty.active
          edit.insert(emptyActive, value)
          editor.selection = new vscode.Selection(emptyActive, emptyActive)
        }
      } else {
  ...
  async ctrlPlusn(editor: TextEditor, edit: TextEditorEdit) {
    ...
    if (selections.length === 1 || new Set(selections.map((e) => e.start.line)).size === 1) {
